...
## merge primary, secondary, text, metadata
eth <- cbind(eth.prim,
             eth.sec[-match('indx', colnames(eth.sec))],
             eth.text[-match('indx', colnames(eth.text))],
             eth.scrape[-match('indx', colnames(eth.scrape))]
             )
eth$text <- stringi::stri_trans_general(eth$text, 'latin-ascii')
eth$cite_evaluation <-
  tolower(gsub('[ \\-0-9]', '', eth$cite_evaluation, perl = TRUE))

## culture metadata, including region, glotto family, etc
eth.meta <- read.csv(file.path(data.dir, 'NHSEthnography_Metadata.csv'))
cultures <- read.csv(file.path(data.dir, 'NHSCultures_Metadata.csv'))
...
cultures$hraf_subregion <- eth.meta$hraf_subregion[
  match(cultures$id_hraf, eth.meta$id_hraf)
]
cultures$hraf_subsistence <- eth.meta$hraf_subsistence[
  match(cultures$id_hraf, eth.meta$id_hraf)
]

## merge culture metadata
eth$ow_nw <- cultures$ow_nw[match(eth$id_nhs, cultures$id_nhs)]
eth$nhs_region <- cultures$nhs_region[match(eth$id_nhs, cultures$id_nhs)]
eth$glotto_family <- cultures$glotto_family[match(eth$id_nhs, cultures$id_nhs)]
eth$hraf_region <- cultures$hraf_region[match(eth$id_nhs, cultures$id_nhs)]
eth$hraf_subregion <- cultures$hraf_subregion[match(eth$id_nhs, cultures$id_nhs)]
eth$hraf_subsistence <- cultures$hraf_subsistence[match(eth$id_nhs, cultures$id_nhs)]
...
## export
write.csv(data.frame(culture = names(cites.per.culture),
                     ncites = as.integer(cites.per.culture),
                     npassages = as.integer(passages.per.culture),
                     nwords = tapply(sapply(words, length),
                                     eth$id_nhs,
                                     sum
                                     )
                     ),
          file.path(results.dir, 'NHSEthnography_SummaryStats.csv'),
          row.names = FALSE
          )
...
## original publication language was english
eth$pubnonenglish <- as.integer(eth.scrape$cite_language != 'English')
eth$pubnonenglish[eth.scrape$cite_language == ''] <- NA

## add author field
eth$authorfield_science <- grepl(
  'naturalscientst|naturalscientist|technicalpersonnel|civilengineer|astronomer|physicalanthropologist',
  eth$cite_evaluation
)
eth$authorfield_ethno <- grepl(  # the sole museum curator is Gerald Conaty (ethnologist)
  'ethnohistorian|ethnographer|ethnogolist|ethnologist|socialanthropologist|musicologist|humanist|folklorist|museumcurator',
  eth$cite_evaluation
)
eth$authorfield_linguistic <- grepl(
  'linguist',
  eth$cite_evaluation
)
eth$authorfield_education <- grepl(
  'educator',
  eth$cite_evaluation
)
eth$authorfield_art <- grepl(
  'arthistorian|artisan|artist|artpatron',
  eth$cite_evaluation
)
eth$authorfield_medicine <- grepl(
  'psychologist|psychiatrist|physician',
  eth$cite_evaluation
)
eth$authorfield_business <- grepl(
  'businessman',
  eth$cite_evaluation
)
eth$authorfield_religion <- grepl(
  'theologian|clergyman|missionary|missionaries',
  eth$cite_evaluation
)
eth$authorfield_govt <- grepl(
  'lawyer|governmentofficial',
  eth$cite_evaluation
)
eth$authorfield_socialscience <- grepl(
  'historian|sociologist|socialscientist|economist|geographer',
  eth$cite_evaluation
)
eth$authorfield_travel <- grepl(
  'traveler|foreignresident',
  eth$cite_evaluation
)
eth$authorfield_local <- grepl(
  'indigene',
  eth$cite_evaluation
)

author.sum <- rowSums(eth[, grep('^authorfield', colnames(eth))])
eth[, grep('^authorfield', colnames(eth))] <- sweep(
  eth[, grep('^authorfield', colnames(eth))],
  1,
  author.sum,
  '/'
)

## export author characteristics
write.csv(
  eth[,
      c('indx',
        colnames(eth)[grep('cite', colnames(eth))],
        'pubyear',
        'pubnonenglish',
        colnames(eth)[grep('author', colnames(eth))]
        )
      ],
  file.path(results.dir, 'NHSEthnography_Authors_Parsed.csv')
  )
...
############################################################################
## merge chains after dropping burn-in; diagnostics below show this is ok ##
############################################################################

mods.bpca <- lapply(famd.fnames, readRDS)

## demonstrate that posterior scores are indeed orthogonal in expectation
cov.bpca <- sapply(1:1000,
                   function(i){
                     cov(mods.bpca[[1]]$mcmc$scores[,,i])
                   },
                   simplify = 'array'
                   )
par(mfrow = c(3, 3))
for (i in 1:3){
  for (j in 1:3){
    plot(density(cov.bpca[i, j,]),
         main = 'covariance between loadings on dimension ' %.% i %.% ' and ' %.% j
         )
    abline(v = 0, lty = 2)
  }
}

## merge chains after burn-in of 200 draws
burn <- 1:200
...
write.csv(data.frame(id_nhs = eth$id_nhs,
          ...
          file.path(results.dir, 'ethno_bpca_scores_final.csv'),
          row.names = FALSE
          )
...
pdf('../results/ethno_bpca_mixing_sigmasq.pdf', 6, 6)
plot(mods.bpca[[1]]$mcmc$sigmasq[examine.ind],
     type = 'l',
     col = cols[1],
     xlab = 'MCMC sample',
     ylab = expression(sigma^2)
     )
lines(mods.bpca[[2]]$mcmc$sigmasq, col = cols[2])
lines(mods.bpca[[3]]$mcmc$sigmasq, col = cols[3])
dev.off()

## examine mixing of means
pdf('../results/ethno_bpca_mixing_mus.pdf', 14, 10)
par(mfrow = c(5, 8), oma = rep(0, 4), mar = rep(0, 4))
for (j in 1:length(feature.names)){
    plot(mods.bpca[[1]]$mcmc$mu[j,examine.ind],
         type = 'l',
         col = cols[1],
         ylim = range(mods.bpca[[1]]$mcmc$mu[j,examine.ind],
                      mods.bpca[[2]]$mcmc$mu[j,examine.ind],
                      mods.bpca[[3]]$mcmc$mu[j,examine.ind]
                      ),
         )
    lines(mods.bpca[[2]]$mcmc$mu[j,],
          col = cols[2]
          )
    lines(mods.bpca[[3]]$mcmc$mu[j,],
          col = cols[3]
          )
    rect(0, -10, 1000, 10, col = '#FFFFFF80')
    text(500,
         mean(range(
           mods.bpca[[1]]$mcmc$mu[j,examine.ind],
           mods.bpca[[2]]$mcmc$mu[j,examine.ind],
           mods.bpca[[3]]$mcmc$mu[j,examine.ind]
         )),
         gsub('(.{1,10})', '\\1\n', substr(colnames(X)[j], 1, 30)),
         cex = 2,
         face = 2,
         col = 'black'
         )
...
for (q in 1:Q){
  pdf(sprintf('../results/ethno_bpca_mixing_W%d.pdf', q), 14, 10)
  par(mfrow = c(5, 8), oma = rep(0, 4), mar = rep(0, 4))
  for (j in 1:length(feature.names)){
    plot(mods.bpca[[1]]$mcmc$W.rotate[j, q, examine.ind],
         type = 'l',
         col = cols[1],
         ylim = range(mods.bpca[[1]]$mcmc$W.rotate[j, q, examine.ind],
                      mods.bpca[[2]]$mcmc$W.rotate[j, q, examine.ind],
                      mods.bpca[[3]]$mcmc$W.rotate[j, q, examine.ind]
                      )
         )
    lines(mods.bpca[[2]]$mcmc$W.rotate[j, q, examine.ind],
          col = cols[2]
          )
    lines(mods.bpca[[3]]$mcmc$W.rotate[j, q, examine.ind],
          col = cols[3]
          )
    rect(0, -10, 1000, 10, col = '#FFFFFF80')
    text(500,
         mean(range(
           mods.bpca[[1]]$mcmc$W.rotate[j, q, examine.ind],
           mods.bpca[[2]]$mcmc$W.rotate[j, q, examine.ind],
           mods.bpca[[3]]$mcmc$W.rotate[j, q, examine.ind]
         )),
         gsub('(.{1,10})', '\\1\n', substr(colnames(X)[j], 1, 30)),
...
## regress ethnography scores on author and publication attributes
author.stars <- laply(
  1:Q,  # latent dimensions to analyze
  function(q){
    ## for each latent dimension, get culture fe posterior
    author.stars.q <- llply(
      ...
      function(draw){
        ...
        mod.star <- lm(
          score.q ~
            ...
            author_female +
            authorfield_science +
            ## authorfield_ethno +  # reference category
            authorfield_linguistic +
            authorfield_education +
            authorfield_art +
            authorfield_medicine +
            authorfield_business +
            authorfield_religion +
            authorfield_govt +
            authorfield_socialscience +
            authorfield_travel +
            authorfield_local
         ,
          data = na.omit(data.frame(
            ...
            pubyear = eth$pubyear,
            pubnonenglish = eth$pubnonenglish,
            author_female = eth$author_female,
            eth[,grep('authorfield', colnames(eth))]
          ))
        )
    ...
    author.stars.q <- do.call(rbind, author.stars.q)
    return(author.stars.q)
  }
)

## interpret author results and format for output
author.summary <- melt(
  data.frame(
    q = 1:Q,
    est = apply(author.stars, c(1, 3), mean),
    cilo = apply(author.stars, c(1, 3), quantile, .025),
    cihi = apply(author.stars, c(1, 3), quantile, .975)
  ),
  id.vars = 'q',
  variable.name = 'coef'
)
author.summary <- author.summary[!grepl('Intercept|pubyear', author.summary$coef),]
author.summary$stat <- gsub('^([^.]+)\\..*', '\\1', author.summary$coef)
author.summary$coef <- gsub('^[^.]+\\.(.*)', '\\1', author.summary$coef)
author.summary <- dcast(author.summary, q + coef ~ stat)
...
ggplot(author.summary,
  ...
  ggtitle('reference (dashed line at zero) is male ethnologist writing in english')

## interpret publication year results and format for output
time.summary <- ldply(
  1:3,
  function(q){
    ldply(1900:2000,
          function(year){
            post <-
              (year - 1950) * author.stars[q,,'pubyear'] +
              (year^2 - 1950^2) * author.stars[q,,'I(pubyear^2)']
            data.frame(q = q,
                       year = year,
                       est = mean(post),
                       cilo = quantile(post, .025),
                       cihi = quantile(post, .975)
                       )
          })
  })

write.csv(time.summary,
          file.path(results.dir, 'ethno_bpca_time_summary.csv'),
          row.names = FALSE
          )
ggplot(time.summary,
       aes(x = year,
  ...
  theme_light(20) +
  xlab('year') +
  ylab('expected change in Bayesian principal component, vs reference') +
  ggtitle('reference (dashed line at zero) is 1950')
...
options(width = 100)
sink(file.path(results.dir, 'ethno_bpca_annotation_to_dimension_final.txt'))
for (q in 1:Q){
    cat('\n', rep('=', 80), '\n\nDIMENSION ', q, '\n\n', sep = '')
...
## bivariate plots of dimension loadings and uncertainty ellipses
cols <- rep('#000000', length(feature.names))
thresh <- .1
dimensions <- c(1, 2)
l2 <- sqrt(rowSums(mod.bpca$mle$W[,dimensions]^2))
cols[l2 > thresh] <- substr(rainbow(sum(l2 > thresh)), 1, 7)
pdf(file.path(results.dir, 'bpca_plot.pdf'), 36, 36)
par(mfcol = c(Q, Q), mar = c(5, 5, 0, 0), oma = rep(0, 4))
for (q1 in 1:Q){
  for (q2 in 1:Q){
    if (q1 == q2){
      frame()
...
###################################
## ethnographer reporting biases ##
###################################

## mu is difference of true mean from reported mean
##   mu.std is in reported standard deviations
##   mu.obs converts back to observation scale
reporting <- data.frame(
    variable = substr(names(mod.bpca$mle$mu), 1, 50),
    missing = colMeans(is.na(X)),
    mu.report = colMeans(X, na.rm = TRUE),
    mu.missing = (
      rowMeans(mod.bpca$mcmc$mu) -
        colMeans(X, na.rm = TRUE) * (1 - colMeans(is.na(X)))
    ) / colMeans(is.na(X)),
    mu.true = rowMeans(mod.bpca$mcmc$mu),
    cilo.true = apply(mod.bpca$mcmc$mu, 1, quantile, .025),
    cihi.true = apply(mod.bpca$mcmc$mu, 1, quantile, .975),
    p = ## round(
        pval.1side.to.2side(rowMeans(
            mod.bpca$mcmc$mu > colMeans(X, na.rm = TRUE) # recycle over columns
        ))## ,
        ## 3
    ## )
)
rownames(reporting) <- NULL

reporting$bias <- reporting$mu.report - reporting$mu.true
reporting <- reporting[order(abs(reporting$bias)),]

## ethnographers are more likely to report (true mean lower than reported mean)
print(
  reporting[
    reporting$p < .05 & reporting$bias > 0,
    c('variable', 'missing', 'mu.report', 'mu.missing', 'mu.true', 'bias', 'p')
  ],
  digits = 3,
  row.names = FALSE
)

## ethnographers are less likely to report (true mean higher than reported mean)
print(
  reporting[
    reporting$p < .05 & reporting$bias < 0,
    c('variable', 'missing', 'mu.report', 'mu.true', 'mu.missing', 'bias', 'p')
  ],
  digits = 3,
  row.names = FALSE
)

## singers are significantly older in ceremonial contexts
summary(lm(eth$numeric_singer_logage ~ eth$ceremony))

## age is missing at a much higher rate in ceremonial contexts
prop.table(
  table(ceremony = eth$ceremony,
        age.missing = is.na(eth$numeric_singer_logage)
        ),
  margin = 1
)
...
overlap.summary <- ldply(
  sort(unique(eth$id_nhs)),
  function(id){
    ## identify the groups to which this culture belongs
    region <- cultures$nhs_region[cultures$id_nhs == id]
    glotto <- cultures$glotto_family[cultures$id_nhs == id]
    world <- cultures$ow_nw[cultures$id_nhs == id]
    hraf.region <- cultures$hraf_region[cultures$id_nhs == id]
    hraf.subregion <- cultures$hraf_subregion[cultures$id_nhs == id]
    hraf.subsistence <- cultures$hraf_subsistence[cultures$id_nhs == id]
    ## loop over ethnography dimensions
    ldply(
      1:Q,
      function(q){
        ## distribution of ethnography scores for this culture and dimension
        culture.distr <- mod.bpca$mcmc$scores.rotate[eth$id_nhs == id, q,]
        culture.distr.lo <- unname(quantile(culture.distr, .025))
        culture.distr.hi <- unname(quantile(culture.distr, .975))
        ## average value of various other groups,
        ##   excluding this culture and those like it (per various groupings)
        other.culture.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$id_nhs != id, q,],
          na.rm = TRUE
        )
        other.nhs.region.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$nhs_region != region, q,],
          na.rm = TRUE
        )
        other.glotto.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$glotto_family != glotto, q,],
          na.rm = TRUE
        )
        other.world.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$ow_nw != world, q,],
          na.rm = TRUE
        )
        other.hraf.region.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$hraf_region != hraf.region, q,],
          na.rm = TRUE
        )
        other.hraf.subregion.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$hraf_subregion != hraf.subregion, q,],
          na.rm = TRUE
        )
        other.hraf.subsistence.mean <- mean(
          mod.bpca$mcmc$scores.rotate[eth$hraf_subsistence != hraf.subsistence, q,],
          na.rm = TRUE
        )
        ## return
        return(data.frame(
          dim = 'q.' %.% q,
          culture = id,
          distr.lo = culture.distr.lo,
          distr.hi = culture.distr.hi,
          other.culture.mean,
          other.nhs.region.mean,
          other.glotto.mean,
          other.world.mean,
          other.hraf.region.mean,
          other.hraf.subregion.mean,
          other.hraf.subsistence.mean
        ))
      })
  })

## combine all computed culture statistics
culture.summary <- merge(mean.summary, sd.summary, by = c('dim', 'culture'))
culture.summary <- merge(culture.summary, overlap.summary, by = c('dim', 'culture'))
## append culture metadata
ind <- match(culture.summary$culture, cultures$id_nhs)
culture.summary$nhs_region <- cultures$nhs_region[ind]
culture.summary$hraf_region <- cultures$hraf_region[ind]
culture.summary$hraf_subregion <- cultures$hraf_subregion[ind]
culture.summary$hraf_subsistence <- cultures$hraf_subsistence[ind]
## to show that cultures with more cites are less extreme
...
write.csv(
  culture.summary,
  file.path(results.dir, 'ethno_bpca_culture_summary_final.csv'),
  row.names = FALSE
)
## quick plot
ggplot(overlap.summary,
  ...
  geom_point(aes(y = other.culture.mean, color = 'mean excluding this culture')) +
  geom_point(aes(y = other.nhs.region.mean, color = 'mean excluding this culture\'s nhs region')) +
  geom_point(aes(y = other.hraf.region.mean, color = 'mean excluding this culture\'s hraf region')) +
  geom_point(aes(y = other.hraf.subregion.mean, color = 'mean excluding this culture\'s hraf subregion')) +
  geom_point(aes(y = other.glotto.mean, color = 'mean excluding this culture\'s glotto family')) +
  geom_point(aes(y = other.world.mean, color = 'mean excluding this culture\'s ow/nw')) +
  geom_point(aes(y = other.hraf.subsistence.mean, color = 'mean excluding this culture\'s subsistence status')) +
  coord_flip() +
...
## convergence of culture mean to global mean is much more strongly
##   related to number of documents, not number of passages
summary(
  lm(abs(mean.est) ~ ncites + npassages,
     mean.summary[mean.summary$dim == 'q.1',]
     )
)
summary(
  lm(abs(mean.est) ~ ncites + npassages,
     mean.summary[mean.summary$dim == 'q.2',]
     )
)
summary(
  lm(abs(mean.est) ~ ncites + npassages,
     mean.summary[mean.summary$dim == 'q.3',]
     )
)
...
write.csv(
  docmean.summary,
  file.path(results.dir, 'ethno_bpca_doc_summary_final.csv'),
  row.names = FALSE
)
...
ggplot(docmean.summary) +
  ...
  facet_grid(dim ~ .) +
  ## guides(color = guide_legend(title = '#books')) +
...
write.csv(
  ...
  file.path(results.dir, 'ethno_bpca_within_between_ratios_final.csv')
  )
